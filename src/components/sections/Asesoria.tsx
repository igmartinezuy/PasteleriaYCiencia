...
export default function Asesoria() {
  ...
  return (
    <section id='asesoria' style={{ background: '#F0EDE8', padding: '120px 80px' }}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 96, alignItems: 'start', marginBottom: 64 }}>
        <div>
          ...
          <h2 style={{ fontFamily: 'Montserrat, sans-serif', fontSize: 'clamp(34px, 4vw, 62px)', fontWeight: 700, lineHeight: 1.05, letterSpacing: '-0.02em', color: '#1D1D1B', marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, paddingTop: 8 }}>
          <div style={{ background: 'white', border: '1px solid rgba(29,29,27,0.09)', borderRadius: 12, padding: '20px 24px' }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: '#1D1D1B', marginBottom: 4 }}>Respuesta en 48hs</div>
            <div style={{ fontSize: 13, fontWeight: 300, color: 'rgba(29,29,27,0.55)' }}>Te escribimos personalmente.</div>
          </div>
          <div style={{ background: 'white', border: '1px solid rgba(29,29,27,0.09)', borderRadius: 12, padding: '20px 24px' }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: '#1D1D1B', marginBottom: 4 }}>Sin compromiso</div>
            <div style={{ fontSize: 13, fontWeight: 300, color: 'rgba(29,29,27,0.55)' }}>Primera consulta para entender tu situacion.</div>
          </div>
          <div style={{ background: 'white', border: '1px solid rgba(29,29,27,0.09)', borderRadius: 12, padding: '20px 24px' }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: '#1D1D1B', marginBottom: 4 }}>Experiencia comprobada</div>
            <div style={{ fontSize: 13, fontWeight: 300, color: 'rgba(29,29,27,0.55)' }}>Campeones de America con mas de 10 anos asesorando.</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 680, margin: '0 auto' }}>
        <div style={{ display: 'flex', marginBottom: 48, border: '1px solid rgba(29,29,27,0.12)', borderRadius: 100, overflow: 'hidden' }}>
          {['1 · Tu proyecto', '2 · Tus datos', '3 · Listo'].map((s, i) => (
            <div key={i} style={{ flex: 1, padding: '12px 16px', textAlign: 'center', fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', background: step === i + 1 ? '#1D1D1B' : 'transparent', color: step === i + 1 ? '#F7F6F4' : step > i + 1 ? '#1B7BA0' : 'rgba(29,29,27,0.32)', borderRight: i < 2 ? '1px solid rgba(29,29,27,0.12)' : 'none', transition: 'all 0.3s' }}>
              {s}
            </div>
          ))}
        </div>
        ...
        {step === 1 && (
          <div>
            ...
            {tipos.map((t) => (
              <div key={t.id} onClick={() => setTipo(t.id)} style={{ border: tipo === t.id ? '2px solid #1B7BA0' : '1.5px solid rgba(29,29,27,0.12)', borderRadius: 14, padding: '22px 26px', marginBottom: 12, cursor: 'pointer', background: tipo === t.id ? 'rgba(27,123,160,0.06)' : 'white', transition: 'all 0.2s', display: 'flex', alignItems: 'flex-start', gap: 16 }}>
                ...
                <div>
                  <div style={{ fontSize: 16, fontWeight: 700, color: '#1D1D1B', marginBottom: 5 }}>{t.name}</div>
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
              <div>
                ...
                <input value={nombre} onChange={e => setNombre(e.target.value)} placeholder='Tu nombre' style={{ width: '100%', background: 'white', border: '1.5px solid rgba(29,29,27,0.12)', borderRadius: 10, padding: '12px 16px', fontSize: 14, color: '#1D1D1B', outline: 'none', fontFamily: 'Montserrat, sans-serif' }} />
              </div>
              <div>
                ...
                <input value={email} onChange={e => setEmail(e.target.value)} type='email' placeholder='tu@email.com' style={{ width: '100%', background: 'white', border: '1.5px solid rgba(29,29,27,0.12)', borderRadius: 10, padding: '12px 16px', fontSize: 14, color: '#1D1D1B', outline: 'none', fontFamily: 'Montserrat, sans-serif' }} />
              </div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
              <div>
                ...
                <input value={pais} onChange={e => setPais(e.target.value)} placeholder='Argentina' style={{ width: '100%', background: 'white', border: '1.5px solid rgba(29,29,27,0.12)', borderRadius: 10, padding: '12px 16px', fontSize: 14, color: '#1D1D1B', outline: 'none', fontFamily: 'Montserrat, sans-serif' }} />
              </div>
              <div>
                ...
                <select value={negocio} onChange={e => setNegocio(e.target.value)} style={{ width: '100%', background: 'white', border: '1.5px solid rgba(29,29,27,0.12)', borderRadius: 10, padding: '12px 16px', fontSize: 14, color: negocio ? '#1D1D1B' : 'rgba(29,29,27,0.35)', outline: 'none', fontFamily: 'Montserrat, sans-serif', appearance: 'none', cursor: 'pointer' }}>
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <textarea value={situacion} onChange={e => setSituacion(e.target.value)} placeholder='En que estas trabajando? Que queres lograr? Cuanto mas nos contras, mejor podemos ayudarte.' rows={4} style={{ width: '100%', background: 'white', border: '1.5px solid rgba(29,29,27,0.12)', borderRadius: 10, padding: '12px 16px', fontSize: 14, color: '#1D1D1B', outline: 'none', fontFamily: 'Montserrat, sans-serif', resize: 'vertical' }} />
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <button onClick={() => setStep(1)} style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'rgba(29,29,27,0.42)', background: 'none', border: 'none', cursor: 'pointer' }}>← Volver</button>
              <button onClick={handleSubmit} style={{ fontSize: 12, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#F7F6F4', background: nombre && email && situacion ? '#1D1D1B' : 'rgba(29,29,27,0.2)', padding: '14px 32px', borderRadius: 100, border: 'none', cursor: nombre && email && situacion ? 'pointer' : 'not-allowed', transition: 'all 0.2s' }}>
                Enviar consulta →
              </button>
            </div>
          </div>
        ...
        {step === 3 && (
          <div style={{ textAlign: 'center', padding: '56px 32px' }}>
            <div style={{ width: 64, height: 64, borderRadius: '50%', background: 'rgba(27,123,160,0.12)', border: '1.5px solid rgba(27,123,160,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 24px', fontSize: 24, color: '#1B7BA0' }}>✓</div>
